...
const TIMEOUT = 1000;
const DELAY_RESOLVE_MS = Math.floor(Math.random() * 10000);

console.log("DELAY_RESOLVE_MS", DELAY_RESOLVE_MS, "TIMEOUT", TIMEOUT);
export default {
  getEvents(itemsPerPage, page, fakeReject = false) {
    return new Promise((resolve, reject) => {
      if (!fakeReject) {
        ...
        if (DELAY_RESOLVE_MS > TIMEOUT) {
          setTimeout(() => {
            reject(new Error("The API didn't respond"));
          }, DELAY_RESOLVE_MS);
        } else {
          setTimeout(() => {
            resolve({
